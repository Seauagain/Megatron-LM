...
Four guards added by the 2026-09-12 review against the reference port (PR #7224, design
comparison only, see PROVENANCE.md): `activation_func_clamp_value` must be set (the released
...
official code; the reference port uses a stable sort, which only changes bit-reproducibility
of tied selections across CP or chunk sizes. The pipeline handoff still carries `H_pre` in the
