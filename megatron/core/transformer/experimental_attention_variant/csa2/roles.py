...
``ModelArgs`` / ``Attention`` / ``Indexer``); field naming is kept compatible with the
Megatron-LM draft PR #7224 so a later upstream merge does not have to rename options. The
code here is an independent implementation; see ``docs/dsv41/PROVENANCE.md``.
"""
